...
# user 테이블 전체를 조회
    ...
    # sql = "select code_detail_id, code_detail_name from BCDD.bcd_code_detail"
    # sql = sql + " where code_group_id = 'ROLE' and and is_use = 'Y'"
    # sql = sql + " order by cd_order;" 
    cursor = conn.cursor()
    cursor.execute("select code_detail_id, code_detail_name from BCDD.bcd_code_detail where code_group_id = 'ROLE' and is_use = 'Y' order by cd_order;")
